...
from xtuner.model.utils import prepare_inputs_labels_for_multimodal, \
    prepare_multi_subimages_inputs_labels_for_multimodal
...
def parse_args():
    ...
    parser.add_argument(
        '--model_name_or_path', default='/mnt/pfs-guan-ssai/cv/sunhaoyi/dolphin-2.9.1-yi-1.5-34b',
    ...
    adapter_group.add_argument(
        '--llava',
        #default='/mnt/pfs-guan-ssai/cv/jucic/my_code/internlm/work_dirs/dolphinyi34b-siglipdino-123m-ft-0728-440k-338kgen/epoch_1_hf',
        #default='/mnt/pfs-guan-ssai/cv/jucic/my_code/internlm/work_dirs/dolphinyi34b-siglipdino-123m-ft-0805-606k-338kgen-150knlu/epoch_1_hf',
        # default='/mnt/pfs-mc0p4k/cv/team/jucic/xtuner/work_dirs/llava_dolphin_yi_34b_siglipdino_lora_e1_gpu8_finetune_multisubimages/dolphinyi34b-siglipdino-123m-ft-0820_all_sft_1489k_538kgen_100knlu_total-sharding/iter_6205_hf',
        default=' /mnt/pfs-mc0p4k/cv/team/jucic/xtuner/work_dirs/llava_dolphin_yi_34b_siglipdino_lora_e1_gpu8_finetune_multisubimages/dolphinyi34b-siglipdino-123m-ft-0826_all_sft_1798k_800kgen_150knlu_total/iter_2810_hf',
        help='llava name or path')
    parser.add_argument(
        '--visual-encoder', default='/mnt/pfs-guan-ssai/cv/yanghongfu/siglip-so400m-patch14-384',
        help='visual encoder name or path')
    parser.add_argument(
        '--dino-visual-encoder',
        default='/mnt/pfs-mc0p4k/cv/team/yanghongfu/mindgpt3ov/models/dinov2-large',
        help='visual encoder name or path')
    parser.add_argument(
    ...
    system_group.add_argument(
        # '--system', default='你是一个名字叫做理想同学的AI数字生命体。\n作为一款多模态AI，你有处理和分析图像的能力。\n', help='Specify the system text')
        '--system', default='你是一个名字叫做理想同学的AI数字生命体。\n作为一款多模态AI，你有处理和分析图像的能力。你必须非常谨慎的审查对话中的图像，并在构建答案时考虑问题内容。\n对于非常简单的问题，你应该提供简洁的回答，但对于复杂和开放式的问题，则应提供详尽准确的回答', help='Specify the system text')
...
def main():
    ...
    if args.llava is not None:
        ...
        if 'EVA' in visual_encoder_path:
            ...
            if dynamic_image_size is not None:
                ...
                visual_encoder = AutoModel.from_pretrained(visual_encoder_path, config=hf_config,
                                                           torch_dtype=TORCH_DTYPE_MAP[args.torch_dtype],
                ...
                position_embedding_weight = resample_abs_pos_embed(
                    ...
                    (dynamic_image_size // visual_encoder.vision_model.embeddings.patch_size,
                     dynamic_image_size // visual_encoder.vision_model.embeddings.patch_size),
            ...
            if dynamic_image_size is not None:
                ...
                embedding_size = pos_embed.shape[-1]
                orig_size = int(
                    visual_encoder.vision_model.embeddings.image_size / visual_encoder.vision_model.embeddings.patch_size)
        ...
        if dynamic_image_size is not None:
            ...
            visual_encoder.vision_model.embeddings.register_buffer("position_ids",
        ...
        if dynamic_image_size is not None:
            image_processor = CLIPImageProcessor.from_pretrained(visual_encoder_path, crop_size=dynamic_image_size,
    ...
    def inference(image, text):
        ...
        if args.image is None:
            ...
            if num_sub_images > 0:
                ...
                # images = dynamic_preprocess(image, image_size=input_size, use_thumbnail=True,min_num=4, max_num=4)
    ...
    model_name = '0828'
    results_xlsx_path_abs = '/mnt/pfs-guan-ssai/cv/yanghongfu/xtuner-main/sft_result_new/{}'.format(model_name)
    os.makedirs(results_xlsx_path_abs, exist_ok=True)

    excel_path = '/mnt/pfs-guan-ssai/cv/yanghongfu/xtuner-main/sft_result_new/0826badcase.csv'
    # images_path = '/mnt/pfs-guan-ssai/cv/jucic/my_code/internlm/sft_test_new/产品泛化集/images'
    results_xlsx_path = os.path.join(results_xlsx_path_abs, 'result_{}_0826allbadcase.xlsx'.format(model_name))
    csv_info = pd.read_csv(excel_path).ffill()
    all_infos = [["index", "question", "answer"]]
    for index, row in csv_info.iterrows():
        id = row['file_key']
        if os.path.exists(id):
            image_path = id
        else:
            print('no image for id:{}'.format(id))
            all_infos.append([id, question, 'no image found'])
            continue
        if os.path.exists(image_path):
            question = row['query泛化']
            question = question.replace('\t', '')
            question = question.replace('\n', '')
            answer = inference(image_path, question)
            answer = answer.replace('\t', '')
            answer = answer.replace('\n', '')
            all_infos.append([id, question, answer])
    results_df = pd.DataFrame(all_infos)
    with pd.ExcelWriter(results_xlsx_path, engine='openpyxl') as writer:
        results_df.to_excel(writer, index=False)

    # excel_path = '/mnt/pfs-guan-ssai/cv/yanghongfu/xtuner-main/sft_result_new/OCRbadcae.csv'
    # # images_path = '/mnt/pfs-guan-ssai/cv/jucic/my_code/internlm/sft_test_new/产品泛化集/images'
    # results_xlsx_path = os.path.join(results_xlsx_path_abs, 'result_{}_OCRbadcase.xlsx'.format(model_name))
    # csv_info = pd.read_csv(excel_path).ffill()
    # all_infos = [["index", "question", "answer"]]
    # for index,row in csv_info.iterrows():
    #     id = row['file_key']
    ...
    #     results_df.to_excel(writer, index=False)
    #
    # excel_path = '/mnt/pfs-guan-ssai/cv/yanghongfu/xtuner-main/sft_result_new/物badcase.csv'
    # # images_path = '/mnt/pfs-guan-ssai/cv/jucic/my_code/internlm/sft_test_new/产品泛化集/images'
    # results_xlsx_path = os.path.join(results_xlsx_path_abs, 'result_{}_物badcase.xlsx'.format(model_name))
    # csv_info = pd.read_csv(excel_path).ffill()
    # all_infos = [["index", "question", "answer"]]
    # for index,row in csv_info.iterrows():
    #     id = row['file_key']
