...
def main():
    ...
    if 'LLaVAModel' in model_name:
        ...
        if cfg.model.get('dino_visual_encoder') and (
                not cfg.model.get('freeze_visual_encoder', False)
                or cfg.model.get('visual_encoder_lora')):
            if 'PeftModel' in model.dino_visual_encoder.__class__.__name__:
                dino_visual_encoder_path = osp.join(args.save_dir,
                                               'dino_visual_encoder_adapter')
                print(
                    f'Saving dino_visual_encoder adapter to {dino_visual_encoder_path}')
            else:
                dino_visual_encoder_path = osp.join(args.save_dir, 'dino_visual_encoder')
                print('Saving dino_visual_encoder image_processor to'
                      f'{dino_visual_encoder_path}')
                image_processor = BUILDER.build(cfg.image_processor)
                image_processor.save_pretrained(dino_visual_encoder_path)
                print(f'Saving dino_visual_encoder to {dino_visual_encoder_path}')
            model.dino_visual_encoder.save_pretrained(
                dino_visual_encoder_path, max_shard_size=args.max_shard_size)

        if cfg.model.get('convnext_visual_encoder') and (
                not cfg.model.get('freeze_visual_encoder', False)
                or cfg.model.get('visual_encoder_lora')):
            if 'PeftModel' in model.convnext_visual_encoder.__class__.__name__:
                convnext_visual_encoder_path = osp.join(args.save_dir,
                                               'convnext_visual_encoder_adapter')
                print(
                    f'Saving convnext_visual_encoder adapter to {convnext_visual_encoder_path}')
            else:
                convnext_visual_encoder_path = osp.join(args.save_dir, 'convnext_visual_encoder')
                print('Saving convnext_visual_encoder image_processor to'
                      f'{convnext_visual_encoder_path}')
                image_processor = BUILDER.build(cfg.convnext_image_processor)
                image_processor.save_pretrained(convnext_visual_encoder_path)
                print(f'Saving convnext_visual_encoder to {convnext_visual_encoder_path}')
            model.convnext_visual_encoder.save_pretrained(
                convnext_visual_encoder_path, max_shard_size=args.max_shard_size)
