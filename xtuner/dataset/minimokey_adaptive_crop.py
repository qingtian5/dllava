...
def dynamic_preprocess(image, min_num=1, max_num=12, image_size=448, use_thumbnail=False):
    ...
    print(f"dynamic_preprocess target_aspect_ratio: {target_aspect_ratio}")
...
def dynamic_preprocess2(image, min_num=1, max_num=12, prior_aspect_ratio=None, image_size=448, use_thumbnail=False):
    ...
    print(f"new_target_ratios: {new_target_ratios}")
    ...
    print(f"processed_images: {len(processed_images)}")
...
def load_image(image_file, input_size=448, min_num=1, max_num=12):
    image = Image.open(image_file).convert('RGB').resize((1920, 1080))
...
pixel_values, target_aspect_ratio = load_image('/mnt/pfs-guan-ssai/cv/yanghongfu/xtuner-main/view.jpg', min_num=4, max_num=12)
print(f"pixel_values: {pixel_values.shape}, target_aspect_ratio: {target_aspect_ratio}")
# pixel_values = pixel_values.to(torch.bfloat16).cuda()
pixel_values2 = load_image2('/mnt/pfs-guan-ssai/cv/yanghongfu/xtuner-main/view.jpg', min_num=3, max_num=7, target_aspect_ratio=target_aspect_ratio)
print(f"pixel_values: {pixel_values2.shape}")
# pixel_values2 = pixel_values2.to(torch.bfloat16).cuda()
print(f"pixel_values2[:-1]: {pixel_values2[:-1].shape}, pixel_values[:-1]: {pixel_values[:-1].shape}, pixel_values2[-1:]: {pixel_values2[-1:].shape}")
