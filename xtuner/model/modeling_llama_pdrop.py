...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layer_list = [15, 30, 45]
        self.image_token_ratio_list = [1.0, 0.5, 0.25, 0.125]
    ...
    def pdrop_forward(
        ...
        self.image_tokens = [x[0][1] - x[0][0] - 1 for x in slice_indices]
    ...
    def pdrop_rank_drop(
        ...
        position_ids, attention_mask, labels, slice_indices
    ):
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":
            ...
            self.image_token_posi = [x[0][0] for x in slice_indices]
            self.prompt_len = [x[0][0] for x in slice_indices]

            # print(f"self.image_tokens: {self.image_tokens}")
            # print(f"self.image_token_posi: {self.image_token_posi}")
            # print(f"self.prompt_len: {self.prompt_len}")
            ...
            # print(f"keep_length: {keep_length}")
            ...
            # print(f"eager_attention_mask: {eager_attention_mask.shape}")
        
            # take valid features
            features = [cur_features[cur_attention_mask] for cur_features, cur_attention_mask in zip(features, attention_mask)]
            # print(f"features: {[x.shape for x in features]}")
            labels = [cur_labels[cur_attention_mask] for cur_labels, cur_attention_mask in zip(labels, attention_mask)]
            # print(f"labels: {[x.shape for x in labels]}")
            ...
            # print(f"attention_mask: {[x.shape for x in attention_mask]}")

            # rank & drop
            for i in range(batch_size):
                image_index = self.image_token_posi[i] + 1
                ...
                # print(f"cur_key_states: {cur_key_states.shape}")
                # print(f"cur_query_states: {cur_query_states.shape}")
                # print(f"cur_eager_attention_mask: {cur_eager_attention_mask.shape}")
                ...
                if self.training:
                    ...
                    # print(f"index_before_answer: {index_before_answer}")
                    text_query_states = cur_query_states[:,index_before_answer,:]  
                    # print(f"text_query_states: {text_query_states.shape}")
                    text_eager_attention_mask = cur_eager_attention_mask[:,index_before_answer,:]
                    # print(f"text_eager_attention_mask: {text_eager_attention_mask.shape}")

                else:
                    prompt_total_len = self.prompt_len[i] + image_tokens[i] + 2
                    # print(f"prompt_total_len: {prompt_total_len}")
                ...
                # print(f"attention_avg_head: {attention_avg_head.shape}")
                # print(f"image_index:image_index+image_tokens[i]: {image_index, image_index+image_tokens[i]}")
                ...
                top_rank_index = [x.to(features[i].device) for x in top_rank_index]

                start_index = image_index + image_tokens[i]
                # print(f"image_index: {image_index}")
                # print(f"top_rank_index: {len(top_rank_index)}")
                # print(f"start_index: {start_index}")

                new_input_embeds = torch.cat([features[i][ :image_index, :] ,features[i][top_rank_index, :], features[i][start_index:, :]], dim=0)
                # print(f"labels[i]: {labels[i].shape}")
                # print(f"labels[i][ :image_index]: {labels[i][ :image_index].shape}")
                # print(f"labels[i][top_rank_index]: {labels[i][top_rank_index].shape}")
                # print(f"labels[i][start_index:]: {labels[i][start_index:].shape}")
                ...
                new_slice_indices = [[slice_indices[i][0][0], image_index + keep_length[i]]]

                # print(f"new_input_embeds: {new_input_embeds.shape}")
                # print(f"new_attention_mask: {new_attention_mask.shape}")
                ...
                slice_list.append(new_slice_indices)
            ...
            return position_ids, new_attention_mask, new_input_embeds, new_labels, slice_list
        
        else:
            raise ValueError(f"Unexpected tokenizer_padding_side: {self.config.tokenizer_padding_side}")


class LlamaForCausalLM_PDrop(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, slice_indices=None, **kwargs
    ):
