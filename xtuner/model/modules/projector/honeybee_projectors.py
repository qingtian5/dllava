...
class Projector(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.prenorm is not None:
            # print(f"do self.prenorm")
            x = self.prenorm(x)

        if self.pos_emb is not None:
            # print(f"do self.pos_emb")
            x = x + self.pos_emb
...
class CAbstractor(ConvProjector):
    ...
    def build_net(self):
        ...
        # self.cascaded = self.config.cascaded
        # self.num_sub_images = self.config.num_sub_images
