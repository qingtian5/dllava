...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 384) -> None:
        ...
        self.dino_featurizer: VisionTransformer = timm.create_model(
            ...
            pretrained_cfg_overlay=dict(file='/mnt/pfs-guan-ssai/cv/yanghongfu/xtuner-main/timm--vit_large_patch14_reg4_dinov2.lvd142m.model.safetensors')
        )
        self.dino_featurizer.eval()

        self.siglip_featurizer: VisionTransformer = timm.create_model(
            self.siglip_timm_path_or_url, pretrained=True, num_classes=0, img_size=self.default_image_size,
            pretrained_cfg_overlay=dict(file='/mnt/pfs-guan-ssai/cv/yanghongfu/xtuner-main/timm--ViT-SO400M-14-SigLIP-384.open_clip_model.safetensors')
        )
        ...
        # print(f"self.dino_featurizer.blocks: {len(self.dino_featurizer.blocks)}, {len(self.dino_featurizer.blocks) - 2}")
        # print(f"self.siglip_featurizer.blocks: {len(self.siglip_featurizer.blocks)}, {len(self.siglip_featurizer.blocks) - 2}")
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        print(f"dino_patches: {dino_patches.shape}, {dino_patches.mean()}, {dino_patches.dtype}")
        siglip_patches = self.siglip_featurizer(pixel_values["siglip"])
        print(f"siglip_patches: {siglip_patches.shape}, {siglip_patches.mean()}, {siglip_patches.dtype}")
...
if __name__ == "__main__":
    model = DinoSigLIPViTBackbone('dinosiglip-vit-so-384px', 'resize-naive')

    image_transform = model.image_transform

    print(image_transform)

    image = Image.open('/mnt/pfs-guan-ssai/cv/yanghongfu/xtuner-main/demo.jpg').convert('RGB')
    pixel_values = image_transform(image)
    pixel_values = {k: v[None, ...] for k, v in pixel_values.items()}
    print(f"pixel_values['dino']: {pixel_values['dino'].shape}, pixel_values['siglip']: {pixel_values['siglip'].shape}")

    patch_features = model({k: pixel_values[k] for k in pixel_values})

    print(f"patch_features: {patch_features.shape}, {patch_features.mean()}")
    ...
    visual_outputs = visual_encoder(pixel_values['siglip'], output_hidden_states=True)
    siglipoutput = visual_outputs.hidden_states[-2]

    print(f"siglipoutput: {siglipoutput.shape}, {siglipoutput.mean()}, {siglipoutput.dtype}")

    dino_visual_outputs = dino_visual_encoder(pixel_values['dino'], output_hidden_states=True)
    dinooutput = dino_visual_outputs.hidden_states[-2][:, 1:]

    print(f"dinooutput: {dinooutput.shape}, {dinooutput.mean()}, {dinooutput.dtype}")
