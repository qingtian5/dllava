...
class Idefics2Model(Idefics2PreTrainedModel):
    ...
    def forward(
        ...
        if pixel_values is not None and image_hidden_states is not None:
            ...
            print(f"pixel_attention_mask: {pixel_attention_mask.shape}")

            patch_size = self.config.vision_config.patch_size
            print(f"patch_size: {patch_size}")
            patches_subgrid = pixel_attention_mask.unfold(dimension=1, size=patch_size, step=patch_size)
            print(f"patches_subgrid: {patches_subgrid.shape}")
            patches_subgrid = patches_subgrid.unfold(dimension=2, size=patch_size, step=patch_size)
            print(f"patches_subgrid: {patches_subgrid.shape}")
            patch_attention_mask = (patches_subgrid.sum(dim=(-1, -2)) > 0).bool()
            print(f"patch_attention_mask: {patch_attention_mask.shape}")
            print(f"patch_attention_mask: {patch_attention_mask[0].shape}\npatch_attention_mask: {patch_attention_mask[0]}")
...
if __name__ == '__main__':
    ...
    processor = AutoProcessor.from_pretrained("/mnt/pfs-guan-ssai/cv/yanghongfu/idefics2-8b")
    image1 = load_image("Statue-of-Liberty-Island-New-York-Bay.jpg")
    # image2 = load_image("https://cdn.britannica.com/59/94459-050-DBA42467/Skyline-Chicago.jpg")
    # image3 = load_image("https://cdn.britannica.com/68/170868-050-8DDE8263/Golden-Gate-Bridge-San-Francisco.jpg")
    ...
    model = Idefics2ForConditionalGeneration.from_pretrained("/mnt/pfs-guan-ssai/cv/yanghongfu/idefics2-8b")
    model = model.cuda()
