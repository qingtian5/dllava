...
from xtuner.dataset.utils import expand_original_aspect
...
def get_img_dict(images_path):
    img_dict = {}
    for root, dirs, files in os.walk(images_path):
        for file in files:
            if file.lower().endswith(
                    ('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff')):
                image_basename = file.split('.')[0]
                image_path = os.path.join(root, file)
                img_dict[image_basename] = image_path
    return img_dict
...
def parse_args():
    ...
    adapter_group.add_argument(
        '--llava',
        default='/mnt/pfs-mc0p4k/cv/team/lishanshan/data/code_repo/Friday_SFT/xtuner-new-lishanshan/work_dirs/stage3_llava_mindgpt_moe_siglipdino_lora_e1_gpu8_finetune_multisubimages_new/1122_sft_car_260k_short_a_pure_t_shuffle/iter_558_hf',
        help='llava name or path')
    parser.add_argument(
        '--results_xlsx_path',
        default='11-20_car_test_result_260k_sft_acc_2_no_crop_1123.xlsx',
        help='the path of xlsx result')
    parser.add_argument(
        '--test_data',
        default='test',
        help='the type of test_data')
...
def main():
    ...
    excel_path = f'/mnt/pfs-mc0p4k/cv/team/lishanshan/data/OV_SFT/Car_sft_data/data/test_data/1120_car_test_train/11-20_car_test.xlsx'
    images_path = f'/mnt/pfs-mc0p4k/cv/team/lishanshan/data/OV_SFT/Car_sft_data/data/test_data/1120_car_test_train/test_images'
    results_xlsx_path_abs = '/mnt/pfs-mc0p4k/cv/team/lishanshan/data/code_repo/Friday_SFT/xtuner-new-lishanshan/work_dirs/test_results/MindGPT-3OV/Car_zice'
    if not os.path.exists(results_xlsx_path_abs):
        os.mkdir(results_xlsx_path_abs)
    results_xlsx_path = os.path.join(results_xlsx_path_abs, args.results_xlsx_path)
    if excel_path.endswith('.xlsx'):
        csv_info = pd.read_excel(excel_path).ffill()
    else:
        csv_info = pd.read_csv(excel_path).ffill()
    img_dict = get_img_dict(images_path)
    all_infos = [["image_path", "query", "predict"]]
    for row in csv_info.index.values:
        image_basename = csv_info.iloc[row, 0].split('.')[0].split('/')[-1]
        question = csv_info.iloc[row, 2]
        question = question.replace('\t', '')
        question = question.replace('\n', '')
        index = image_basename
        if image_basename in img_dict.keys():
            image_path = img_dict[image_basename]
            ...
            original_image = cv2.imread(image_path).astype(np.uint8)
            ...
            print("width, height: ", image.shape[1], image.shape[0])
            ...
            answer = answer.replace('\n', '')
            all_infos.append([index, question, answer])
        else:
            all_infos.append([index, question, '---图片丢失，请输入正确图片---'])
